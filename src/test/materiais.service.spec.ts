...
describe("Materiais Service", () => {
  ...
  beforeAll(async () => {
    ...
    const moduleRef = await Test.createTestingModule({
      providers: [
        ...
        {
          provide: getRepositoryToken(MaterialEntity, "postgreConnection"),
  ...
  it("Insert Material", async () => {
    ...
    const created = { ...baseMaterial, id: undefined } as unknown as MaterialEntity;
    materialRepo.create.mockReturnValue(created);
    envioRepo.findOne.mockResolvedValue(baseEnvio);
    materialRepo.save.mockImplementation(async (entity: MaterialEntity) => ({
      ...entity,
      id: baseMaterial.id,
    ...
    expect(envioRepo.findOne).toHaveBeenCalledWith({ where: { id: dto.envio_id } });
  ...
  it("Get Materiais", async () => {
    ...
    expect(materialRepo.find).toHaveBeenCalledWith({ order: { id: "DESC" } });
  ...
  it("Get Materiais By Envio", async () => {
    ...
    const response = await materiaisService.getMateriaisByEnvio(baseEnvio.id);
  ...
  it("Get Material", async () => {
    ...
    const response = await materiaisService.getMaterial(baseMaterial.id, { withEnvio: true });
  ...
  it("Update Material", async () => {
    ...
    materialRepo.save.mockImplementation(async (entity: MaterialEntity) => ({ ...entity, updated_at: otherEnvio.updated_at }));
  ...
  it("Delete Material", async () => {
    ...
    expect(materialRepo.findOne).toHaveBeenCalledWith({ where: { id: baseMaterial.id } });
